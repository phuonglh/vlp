...
/**
  * Utils for handling schemas, copied from the Spark framework.
  *
  * phuonglh
  */
object SchemaUtils {
  ...
  /**
    ...
    */
  def checkColumnTypes(schema: StructType, colName: String, dataTypes: Seq[DataType], msg: String = ""): Unit = {
    val actualDataType = schema(colName).dataType
    val message = if (msg != null && msg.trim.length > 0) " " + msg else ""
