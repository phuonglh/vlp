 ...
 def buildModel(inputShape: Shape): Sequential[Float] = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val opts = new OptionParser[Config](getClass().getName()) {
      ...
      opt[Int]('h', "hiddenSize").action((x, conf) => conf.copy(hiddenSize = x)).text("number of hidden units in each layer")
      ...
      opt[Double]('u', "dropoutProbability").action((x, conf) => conf.copy(dropoutProbability = x)).text("dropout ratio, default is 0")
