...
object WordEmbeddingP {
  ...
  def getOutputDimFromEmbeddingFile(embeddingFile: String): Int = {
    embeddingFile.split("/").last match {
      ...
      case "glove.6B.100d.vocab.txt" => 100       // phuonglh hack to use only a small vocab of DEP
      case "glove.6B.200d.txt" => 200
      case "glove.6B.300d.txt" => 300
      case "glove.42B.300d.txt" => 300
      case "glove.840B.300d.txt" => 300
      case "numberbatch-en-19.08.txt" => 300
      case "numberbatch-en-19.08.vocab.txt" => 300 // phuonglh hack to use only a small vocab of DEP
      case "cc.id.300.vocab.vec" => 300 // phuonglh
      case "numberbatch-vi-19.08.vocab.txt" => 300 // phuonglh
      case _ =>
