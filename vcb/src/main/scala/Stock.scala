...
import com.intel.analytics.bigdl.dllib.optim.{Adam, Loss, MAE, Trigger}
...
object Stock {
  ...
  def preprocess(af: DataFrame, config: ConfigStock): DataFrame = {
...
//      .set("spark.driver.memory", config.driverMemory)
    ...
    val af = spark.read.options(Map("header" -> "true", "inferSchema" -> "true")).csv("dat/HOSE_2008_2022_vnstock.csv").filter(col("Stock") === "VNM")
    af.show(false)
    println(af.count())
...
//    println(s"r2: ${trainingSummary.r2}")
    ...
    val estimator = NNEstimator(bigdl, MSECriterion(), Array(25), Array(1))
    estimator.setLabelCol("y").setFeaturesCol("x")
      ...
      .setValidation(Trigger.everyEpoch, df, Array(new MAE()), config.batchSize)
    estimator.fit(df)
