...
/**
  * phuonglh, April 2021
  * 
  * A topic modeling tool for processing documents.
  * 
  */
class TopicModel(val sparkContext: SparkContext, val config: ConfigLDA) {
  ...
  def createDataset(jsonPath: String): Dataset[Document] = {
    val data = TopicModel.tokenize(sparkContext, jsonPath).toDF("category", "text").as[Document]
...
object TopicModel {
  ...
  def tokenize(sparkContext: SparkContext, inputPath: String): RDD[Document] = {
    ...
    logger.info("Start segmenting the news into sentences to build document collection...")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = new OptionParser[ConfigLDA]("vlp.tmi") {
      head("vlp.tmi", "1.0")
      ...
      opt[String]('d', "dataPath").action((x, conf) => conf.copy(dataPath = x)).text("data path, default is 'dat/fin/24.json'")
    ...
    parser.parse(args, ConfigLDA()) match {
      case Some(config) => {
        ...
        val lda = new TopicModel(sparkSession.sparkContext, config)
        config.mode match {
          case "train" => {
